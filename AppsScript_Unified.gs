...
function getScenarioDefinition(sid) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (!result.highLevelEn) result.highLevelEn = String(row[1] || "").trim();
    if (!result.highLevelKo) result.highLevelKo = String(row[2] || "").trim();
    var id = padSlot_(String(row[3] || "").trim());
    if (id) result.items.push({ id: id, name: String(row[4] || "").trim(), quantity: row[5] || 1 });
  }
