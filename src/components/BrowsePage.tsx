      ...
      <div style={{ maxWidth: step === "scenario" || step === "warehouse" ? "1200px" : "620px", margin: "0 auto", padding: "24px 16px 96px" }}>
        ...
        {step === "scenario" ? (
          <div className="bsp-scenario-layout">
            ...
            <div className="bsp-scenario-main">
          <ItemGrid C={C} Spinner={Spinner} loaded={sciLoaded} loading={sciLoading} count={sciFiltered.length} total={sciItems.length} error={sciErr} onRetry={() => { setSciErr(""); loadScenario(); }} stickyTop={stickyHeaderHeight}
            filterRow={
              <>
        ...
        {step === "warehouse" ? (
          <ItemGrid C={C} Spinner={Spinner} loaded={whLoaded} loading={whLoading} count={whFiltered.length} total={whItems.length} error={whErr} onRetry={() => { setWhErr(""); loadWarehouse(); }} stickyTop={stickyHeaderHeight}
            filterRow={
              <>
                <div style={{ position: "relative", flex: "2 1 260px", minWidth: 0 }}>
                  <Search size={15} style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", color: C.label }} />
...
function ItemGrid({ C, Spinner, loaded, loading, count, total, filterRow, children, error, onRetry, stickyTop }: any) {
